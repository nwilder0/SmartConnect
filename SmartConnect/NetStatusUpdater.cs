...
namespace SmartConnect
{
    class NetStatusUpdater
    {
        ...
        public void Update()
        {
            if (main.Iface == null) main.SetWirelessConnection();
            
            main.SetState();

            if ((main.State != WiFiConnect.WiFiState.NoWirelessInterface) && (main.Iface != null))
            {
                ...
                WlanClient.WlanInterface iface = main.Iface;
                lock (iface)
                {
                ...
                {
                    main.SetLocation(WiFiConnect.NetLocation.Local);
                }
                else
                {
                    main.SetLocation(WiFiConnect.NetLocation.Foreign);
                }
            ...
            {
                main.SetLocation(WiFiConnect.NetLocation.Unknown);
                // clear out SSIDs, APs and set appropriate status, bandwidth
                main.MainForm.TSSetConnectButton("WiFi Adapter Not Available" + Environment.NewLine + "Ensure WiFi Is Turned On");
                main.MainForm.TSSetSSIDs(new String[0],0);
                main.MainForm.TSSetAPs(new String[0], 0);
            }
        }
    }
}
